...
def check_reward_signal():
    ...
    print(f"  Random policy vs Greedy:")
...
def check_training_loop():
    ...
    if abs(obs_min) < 20 and abs(obs_max) < 20:
        print(f"  [OK] Normalized observations are in reasonable range!")
    else:
        print(f"  [WARN] Normalized observations may still have issues")
    ...
    if update_info['value_loss'] < 10 and update_info['approx_kl'] < 0.1:
        print(f"  [OK] Training metrics look healthy!")
    else:
        print(f"  [WARN] Training metrics may indicate issues")
