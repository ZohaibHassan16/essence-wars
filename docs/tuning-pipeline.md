...
├── train.log                    # Full training log with all generations
├── weights.toml                 # Best weights found (24 parameters)
├── summary.txt                  # Quick stats (fitness, win rate, time)
...
└── plots/                       # Visualizations (generated by analysis)
    ├── overview.png             # Win rate & fitness over time
    ├── efficiency.png           # Time per generation analysis
    ├── convergence.png          # Weight parameter evolution
    ├── comparison.png           # Multi-run comparison (if applicable)
    └── summary_report.txt       # Detailed text analysis
```
...
3. **Weights converge to high win rates anyway**
   - Example: Gen 0 @ 75% WR → Gen 50 @ 88% WR
...
```
┌─────────────┐
│  MctsBot    │  Selects best action via UCB1 tree search
└──────┬──────┘
       │
       ▼
┌─────────────────┐
│   Tree Search   │  For each node, runs simulations:
└────────┬────────┘
         │
         ▼
  ┌─────────────────────┐
  │  Selection (UCB1)   │  Navigate tree to leaf
  └──────────┬──────────┘
             │
             ▼
  ┌─────────────────────┐
  ...
  └──────────┬──────────┘
             ...
             ▼
  ┌─────────────────────┐
  │  Rollout            │  Simulate game to end using GreedyBot
  │  (Uses Tuned        │  ← Tuned weights make this smarter!
  │   Weights!)         │
  └──────────┬──────────┘
             │
             ▼
...
User calls MctsBot.select_action()
  ...
  │     │     ├─► Expansion: Add new node
  │     │     │
  │     │     ├─► Rollout: Play random game to end
  │     │     │   │
  │     │     │   └─► Uses GreedyBot with tuned weights!
  │     │     │       (Makes better decisions during simulation)
  │     │     │
  │     │     └─► Backpropagate: Update win counts
  │     │
  │     └─► Aggregate results
  │
...
Stops when win rate ≥ 90% or 200 generations reached.
