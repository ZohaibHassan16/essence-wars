...
fn test_support_encoding_occupied() {
    ...
    // [2]: card_id = 100.0 / 6000.0 (normalized, 6000 handles commander IDs up to 5011)
    assert_eq!(tensor[support_offset + 2], 100.0 / 6000.0);
}
...
fn test_hand_cards_encoding() {
    ...
    // Hand cards start at offset 5 within player state (normalized by 6000.0)
    assert_eq!(tensor[player1_offset + 5], 1.0 / 6000.0); // Card ID 1 normalized
    assert_eq!(tensor[player1_offset + 6], 5.0 / 6000.0); // Card ID 5 normalized
    assert_eq!(tensor[player1_offset + 7], 10.0 / 6000.0); // Card ID 10 normalized
