<script lang="ts">
  ...
  async function handleStart(config: WizardConfig) {
    const spectatorConfig: SpectatorConfig = {
      player1DeckId: config.playerDeckId,
      player1BotType: config.playerBotType ?? "greedy",
      player2DeckId: config.opponentDeckId,
      player2BotType: config.opponentBotType,
      seed: config.customSeed,
      mctsSimulations: config.mctsSimulations,
      alphabetaDepth: config.alphabetaDepth,
    };

    spectatorStore.setWatchLive(config.watchLive ?? false);
    spectatorStore.setCommentaryEnabled(config.enableCommentary ?? true);
    await spectatorStore.startMatch(spectatorConfig);
  }
...
{#if spectatorStore.isComputing}
  <div class="fixed inset-0 z-40 bg-black/50 flex items-center justify-center">
    <div class="bg-ui-panel rounded-xl p-8 shadow-2xl flex flex-col items-center gap-4">
      ...
      <p class="text-ui-text font-semibold">Computing match...</p>
    </div>
  </div>
{/if}
...
<div class="w-full h-full">
  <DeckSelectionWizard
    mode="ai-vs-ai"
    decks={spectatorStore.decks}
    bots={spectatorStore.bots}
    onStart={handleStart}
    onBack={handleBack}
  />
</div>
