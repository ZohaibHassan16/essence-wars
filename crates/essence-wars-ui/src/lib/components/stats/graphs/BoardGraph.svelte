<script lang="ts">
  ...
  function createChart() {
    ...
    const data: uPlot.AlignedData = [
      timeline.turns,
      timeline.player1BoardPresence,
      timeline.player2BoardPresence,
    ];
    ...
    const opts: uPlot.Options = {
      ...
      height: 350,
      title: "Board Presence Over Time",
      class: "uplot-dark",
      scales: {
        x: { time: false },
        y: {
          auto: true,
          range: [0, Math.ceil(maxPresence * 1.1)],
        },
      },
      axes: [
        {
...
<div class="mt-2 text-xs text-center text-ui-text-dim">
  Board presence = sum of (attack + health) for all creatures on the battlefield
</div>
